 ...
 * @author od
 */
public class SwatPlusDomainDB implements AutoCloseable {
  ...
  final String GET_OPDATA_QUERY = "SELECT sp_op_typ, sp_op_data1, sp_op_data2, sp_op_data3, op_kill_crop "
      + "FROM d_cr2sp_ops WHERE cr_op_id=?";
  final String GET_CROP_QUERY = "SELECT crop_id, sp_crop_id, sp_harv_typ "
      + "FROM d_cr2sp_crops WHERE crop_id=?";
  final String GET_FERT_QUERY = "SELECT short_name, inorganic_n, inorganic_p, organic_n, organic_p, solids "
      + "FROM d_manures WHERE short_name=?";

  public static SwatPlusDomainDB create(Connection conn) throws SQLException {
    String dbName = conn.getMetaData().getDatabaseProductName();
    return new SwatPlusDomainDB(conn);
  }

  private SwatPlusDomainDB(Connection con) {
    this.con = con;
  }
