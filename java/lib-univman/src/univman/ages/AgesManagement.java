...
/**
 * Management and Operations handling for Ages
 *
 * @author od
 */
public class AgesManagement implements ManagementWriter {
...
//  }
   ...
   */
  public record AgesEvent(AgesOp op, LocalDate date, String op_d1, String op_d2, double op_d3) {
  ...
  static final TriConsumer<UniEvent, AgesDomainDB, AgesManagement> PASS = (UniEvent ue, AgesDomainDB db, AgesManagement sm) -> {
    // do nothing
  };

  String name;

  List<AgesEvent> events = new ArrayList<>();
  ...
  void addEvent(AgesEvent event) {
    events.add(event);
  }

  void addEvents(List<AgesEvent> events) {
    for (AgesEvent e : events) {
      addEvent(e);
    }
  }
  ...
  private Set<String> getCrops() {
    return events.stream()
        .filter(e -> e.op == AgesOp.plnt || e.op == AgesOp.harv || e.op == AgesOp.kill)
        .map(AgesEvent::op_d1)
  ...
  private static void writeOp(PrintWriter pw, AgesOp op, int mo, int day, String opData1, String opData2, double opData3) {
  ...
  private static void writeSkipOp(PrintWriter pw, int yr) {
    String f = String.format(fmt0, AgesOp.skip, 0, 0, 0, null, null, 0.0);
    pw.println(f + "  # " + (yr + 1));
  }

  private static void writeOp(PrintWriter pw, AgesOp op, LocalDate date, String opData1, String opData2, double opData3) {
    writeOp(pw, op, date.getMonthValue(), date.getDayOfMonth(), opData1, opData2, opData3);
  }

  private static void writeOp(PrintWriter pw, AgesEvent ev) {
   ...
   * @return
   */
  public static AgesManagement from(List<UniEvent> uEvents, String name, AgesDomainDB db) throws Exception {
    AgesManagement spm = new AgesManagement();
    spm.setName(name);
    ...
    for (UniEvent ue : uEvents) {
      TriConsumer<UniEvent, AgesDomainDB, AgesManagement> evTransformer = conv.get(ue.getTypeName());
   ...
   */
  private static final Map<String, TriConsumer<UniEvent, AgesDomainDB, AgesManagement>> conv = new HashMap<>() {
    {
      /*
       * Fertilizer
       ...
       */
      put(op_amendment_fertilizer.typename(), (UniEvent ue, AgesDomainDB db, AgesManagement sm) -> {
        AgesDomainDB.OP op = db.getOpData(ue.getOpId(), AgesOp.fert.name());

        // optionally add "n" or "p" to fertilizer.frt and change op_data1 below
        // opdata[2] is application type, e.g. "inject", "broadcast", ...
        List<AgesEvent> l = new ArrayList<>();
        ...
        if (fertName != null) {
          ...
          l.add(new AgesEvent(AgesOp.fert, ue.getDate(), fertName, op.data2(), fertAmount * LBAC_TO_KGHA));
          sm.addEvents(l);
          return;
        }
        ...
        if (min_n > 0)
          l.add(new AgesEvent(AgesOp.fert, ue.getDate(), "mi_n", op.data2(), min_n * LBAC_TO_KGHA));

        double min_p = ue.getDouble(min_p_lbs_ac, 0.0);
        if (min_p > 0)
          l.add(new AgesEvent(AgesOp.fert, ue.getDate(), "mi_p", op.data2(), min_p * LBAC_TO_KGHA));
...
//          l.add(new SpEvent(SpOp.fert, ue.getDate(), "or_p", op.data2(), org_p / LBAC_IN_KGHA));
       ...
       * Planting
       */
      put(op_plant.typename(), (UniEvent ue, AgesDomainDB db, AgesManagement sm) -> {
        // just to check the op_id
        AgesDomainDB.OP op = db.getOpData(ue.getOpId(), AgesOp.plnt.name());
        int cropId = ue.getInt(crop_id);
        String[] crop = db.getCrop(cropId);
        sm.addEvent(new AgesEvent(AgesOp.plnt, ue.getDate(), crop[0], null, 0));
       ...
       * Harvest (and  Kill)
       */
      put(op_harvest.typename(), (UniEvent ue, AgesDomainDB db, AgesManagement sm) -> {
        AgesDomainDB.OP op = db.getOpData(ue.getOpId(), AgesOp.harv.name());
        int cropId = ue.getInt(crop_id);
        String[] crop = db.getCrop(cropId);
        if (crop[0] == null)
          throw new RuntimeException("No SP crop for crop_id " + ue.getInt(crop_id));

        sm.addEvent(new AgesEvent(AgesOp.harv, ue.getDate(), crop[0], op.data2(), 0));

        if (op.kill_crop())
          sm.addEvent(new AgesEvent(AgesOp.kill, ue.getDate(), crop[0], null, 0));
       ...
       * Kill
       */
      put(op_kill.typename(), (UniEvent ue, AgesDomainDB db, AgesManagement sm) -> {
        // just to check the op_id
        AgesDomainDB.OP op = db.getOpData(ue.getOpId(), AgesOp.kill.name());
       ...
       * Tillage
       */
      put(op_tillage.typename(), (UniEvent ue, AgesDomainDB db, AgesManagement sm) -> {
        AgesDomainDB.OP op = db.getOpData(ue.getOpId(), AgesOp.till.name());

        sm.addEvent(new AgesEvent(AgesOp.till, ue.getDate(), op.data1(), null, 0));
       ...
       */
      put(op_amendment_pesticide.typename(), (UniEvent ue, AgesDomainDB db, AgesManagement sm) -> {
        AgesDomainDB.OP op = db.getOpData(ue.getOpId(), AgesOp.pest.name());
...
//        l.add(new SpEvent(SpOp.pest, ue.getDate(), "weedar", op.data2(), 0));
       ...
       * Burn
       */
      put(op_burn.typename(), (UniEvent ue, AgesDomainDB db, AgesManagement sm) -> {
        AgesDomainDB.OP op = db.getOpData(ue.getOpId(), AgesOp.burn.name());
        sm.addEvent(new AgesEvent(AgesOp.burn, ue.getDate(), op.data1(), null, 0));
      });

      /*
       * Grazing
       */
      put(op_graze.typename(), (UniEvent ue, AgesDomainDB db, AgesManagement sm) -> {
        AgesDomainDB.OP op = db.getOpData(ue.getOpId(), AgesOp.graz.name());
        sm.addEvent(new AgesEvent(AgesOp.graz, ue.getDate(), op.data1(), null, 0));
      });

      /*
       * Irrigation amount allocation per year
       */
      put(op_irrigation_allocation.typename(), (UniEvent ue, AgesDomainDB db, AgesManagement sm) -> {
